...
func TestPreparedNumericMetadataIsSparse(t *testing.T) {
	...
	// Collation provenance is carried in the embedded Type. The three scalar
	// fields add one aligned word to ordinary expressions; the prepared-number
	// metadata remains an optional pointer.
	require.Equal(t, uintptr(176), unsafe.Sizeof(planpb.Expr{}))
}
