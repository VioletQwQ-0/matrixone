...
func MakeCollationKeyExpr(ctx context.Context, input *pb.Expr, effectiveType pb.Type, format types.KeyFormat) (*pb.Expr, error) {
	...
	if effectiveType.Charset > 255 {
		return nil, moerr.NewInvalidInput(ctx, "unsupported collation key charset")
	}
	part, err := types.ResolveStringKeyPart(types.NewWithCharset(types.T(effectiveType.Id), effectiveType.Width, effectiveType.Scale, uint8(effectiveType.Charset)), format)
	if err != nil {
		return nil, err
	}
	if !part.Transformed() {
