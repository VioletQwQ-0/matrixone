...
// result bitmap is [2]
// encodeNative0900StringVector handles the physical key path used by the
...
func encodeNative0900StringVector(
	v *vector.Vector,
	ps []*types.Packer,
	bitMap *nulls.Nulls,
	compactNulls bool,
	...
	if !typ.Oid.IsMySQLString() ||
		!types.IsNative0900Collation(typ.Charset) {
		return false, nil
	}
	...
	for i := range values {
		...
		if typ.Charset == types.CharsetUTF8MB40900AI {
			scratch = key[:0]
		}
	}
	return true, nil
...
func serialWithCompacted(
	...
	for _, v := range vs {
		vNull := v.GetNulls()
		hasNull := v.HasNull()
		if handled, err := encodeNative0900StringVector(v, ps, bitMap, true); handled {
			if err != nil {
				return nil, err
			}
			continue
...
func serialWithoutCompacted(
	...
	for _, v := range vs {
		...
		if handled, err := encodeNative0900StringVector(v, ps, nil, false); handled {
			if err != nil {
				return nil, err
			}
			continue
...
func compactSingleIndexCol(
	...
	if types.T(v.GetType().Oid).IsMySQLString() &&
		types.IsNative0900Collation(v.GetType().Charset) {
		...
		for i := range values {
			...
			if v.GetType().Charset == types.CharsetUTF8MB40900AI {
				scratch = key[:0]
			}
		}
